...
$.ajax({
  type: 'GET',
  url: '/json/profile.json',//GETting a JSON file acts just like hitting an API
  success: getTweets,
  error: handleError
});
...
function getTweets(profile) {
  ...
  $.ajax({
    type: 'GET',
   url: '/json/tweets.json?id=' + profile.id,
...
function getMentionedUser(tweets) {
  ...
  $.ajax({
    type: 'GET',
  url: '/json/friend.json?id=' + tweets[0].usersMentioned[0].id,
